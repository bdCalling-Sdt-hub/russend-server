...
const validateTransaction = async (req, res, next) => {
  try {
    ...
    if (!phoneNumber) {
      errors.push({
        field: "phoneNumber",
        message: req.t("phoneNumber-required"),
      });
    }
    if (Number(amountToSent) < 0) {
      errors.push({
        field: "amountToSent",
        message: req.t("amountToSent-format-error"),
      });
    }
    if (!ammountToSentCurrency) {
      errors.push({
        field: "ammountToSentCurrency",
        message: req.t("ammountToSentCurrency-required"),
      });
    }
    if (Number(amountToReceive) < 0) {
      errors.push({
        field: "amountToReceive",
        message: req.t("amountToReceive-format-error"),
      });
    }
    if (!amountToReceiveCurrency) {
      errors.push({
        field: "amountToReceiveCurrency",
        message: req.t("amountToReceiveCurrency-required"),
      });
    }
    if (Number(hiddenFees) < -100 || Number(hiddenFees) > 100) {
      errors.push({
        field: "hiddenFees",
        message: req.t("hiddenFees-format-error"),
      });
    }
    if (!paymentMethod) {
      errors.push({
        field: "paymentMethod",
        message: req.t("paymentMethod-required"),
      });
    }
    if (!country) {
    ...
    if (!countryName) {
      errors.push({
        field: "countryName",
        message: req.t("countryName-required"),
      });
    }
